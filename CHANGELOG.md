...
- Production auth, password/PIN hashing, JWT refresh rotation, Redis queues/rate limits, provider integrations, webhook signatures, and full database repository wiring must be completed by the buyer before live use.
- OpenBank NG is source-code software only. It does not provide a banking license, regulated provider access, legal advice, compliance certification, or live payment-rail authorization.
